...
new _koa2.default().use((0, _koaBetterBody2.default)()).use(_koaHbs2.default.middleware({
...
const blogRouter = new _koaRouter2.default().get('/', async ctx => {
  ...
  const { data: [{
    ...
    }] } = await _directus2.default.getItems('blog', {
  ...
  const { data: [{
    ...
    }] } = await _directus2.default.getItems('directus_users', {
    filters: { id: authorID }
  });
